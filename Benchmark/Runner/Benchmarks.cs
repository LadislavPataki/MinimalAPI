...
public class Benchmarks
{
    ...
    private static HttpClient MinimalV2Client { get; } = new WebApplicationFactory<MinimalApiV2.Program>().CreateClient();
    
    private static HttpClient MinimalReprClient { get; } = new WebApplicationFactory<MinimalApiRepr.Program>().CreateClient();
    
    private static HttpClient MinimalReprV2Client { get; } = new WebApplicationFactory<MinimalApiReprV2.Program>().CreateClient();
    
    private static HttpClient MinimalReprV3Client { get; } = new WebApplicationFactory<MinimalApiReprV3.Program>().CreateClient();
    ...
    //[Benchmark]
    public Task MinimalApiRepr()
    {
        var msg = new HttpRequestMessage()
        {
            Method = HttpMethod.Post,
            RequestUri = new Uri($"{MinimalReprClient.BaseAddress}benchmark/ok/123"),
            Content = Payload
        };

        return MinimalReprClient.SendAsync(msg);
    }

    [Benchmark]
    public Task MinimalApiReprV2()
    {
        var msg = new HttpRequestMessage()
        {
            Method = HttpMethod.Post,
            RequestUri = new Uri($"{MinimalReprV2Client.BaseAddress}api/v1/benchmark/ok/123"),
            Content = Payload
        };

        return MinimalReprV2Client.SendAsync(msg);
    }

    //[Benchmark]
    public Task MinimalApiReprV3()
    {
        var msg = new HttpRequestMessage()
        {
            Method = HttpMethod.Post,
            RequestUri = new Uri($"{MinimalReprV3Client.BaseAddress}benchmark/ok/123"),
            Content = Payload
        };

        return MinimalReprV3Client.SendAsync(msg);
    }

    [Benchmark]
    public Task MinimalApiV2()
    {
        var msg = new HttpRequestMessage()
        {
            Method = HttpMethod.Post,
            RequestUri = new Uri($"{MinimalV2Client.BaseAddress}api/v1/benchmark/ok/123"),
            Content = Payload
        };

        return MinimalV2Client.SendAsync(msg);
    }
